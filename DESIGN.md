...
2. **Resolution pass (separate, async).** Mentions are clustered to canonical `entities` via blocking (name/alias normalization) + embedding similarity + a resolution model for ambiguous cases.
...
pgvector + HNSW over `embeddings`, queries **pinned to one embedding model version**, with a **stated similarity floor** so results aren't a hairball. Returns ranked records with tier, source, confidence, and freshness — always **ACL-filtered to the requesting principal before ranking**.
